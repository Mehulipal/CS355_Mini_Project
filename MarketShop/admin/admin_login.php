...
// If POST request method is made...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
  // Check if admID or password is empty 
  if(empty(trim($_POST['admID'])) || empty(trim($_POST['passwd'])))
  {
    $err = "Please enter AdminID & Password";
  }
  // Otherwise, set variables $admID and $passwd
  else
  {
  ...
  if(empty($err))
  {
    // Prepare mysqli query to select the admin details
    $sql = "SELECT * FROM admin WHERE AdmID = ?";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "s", $param_admID);
    $param_admID = $admID;
    ...
    if(mysqli_stmt_execute($stmt))
    {
      ...
      {
        mysqli_stmt_bind_result($stmt, $AdmID, $AdmName, $hashed_passwd, $Department, $Contact);
        if(mysqli_stmt_fetch($stmt))
        {
          ...
          if(!strcmp(sha1($passwd), $hashed_passwd))
          {
            ...
            //Redirect user to admin_home page
            header("location: admin_home.php");                          
          }
      ...
      // If admID doesn't exist in database, login fails
      else
      {
