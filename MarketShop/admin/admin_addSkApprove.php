...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
  ...
  {
    // Prepare mysqli query to check if SkID already exists in database 
    ...
    if($stmt)
    {
      ...
      if(mysqli_stmt_execute($stmt))
      {
        ...
        // If SkID already exists in database, set SkID error
        if(mysqli_stmt_num_rows($stmt) == 1)
        {
          $SkID_err = "This Shopkeeper ID already exists"; 
        }
        // Otherwise, set variable $SkID
        else
        {
  ...
  else
  {
    ...
    if($stmt)
    {
      ...
      if(mysqli_stmt_execute($stmt))
      {
        ...
        // If SID already does not exist in database, set SID error
  ...
  // If there were no errors, go ahead and insert shopkeeper details into the database
  if(empty($SkID_err) && empty($passwd_err) && empty($confirm_passwd_err) && empty($SID_err))
  {
    ...
    if ($stmt)
    {
      ...
      if (mysqli_stmt_execute($stmt))
      {
        // Insert SID & SkID in the relation entity association
  ...
  // Incase of errors, approval fails and alerts are shown 
  else
  {
    if(!empty($SkID_err))
    {
