...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
  ...
  // If there were no errors, go ahead and insert feedback into the database
  if(empty($SID_err) && empty($Score_err))
  {
    ...
    if ($stmt)
    {
      ...
      // If it executes successfully, redirect to admin_login page
      if (mysqli_stmt_execute($stmt))
      {
  ...
  // Incase of errors, feedback fails fails and alerts are shown 
  else
  {
    if(!empty($SID_err))
    {
