...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
  ...
  if(empty($SID_err) && empty($Month_err) && empty($Year_err))
  {
    ...
    $sql = "INSERT INTO request_elec_bill (SID, PayForWhichMonth, PayForWhichYear, PayeeName, Amount) VALUES (?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $sql);
    if ($stmt)
    {
      mysqli_stmt_bind_param($stmt, "iiisi", $param_SID, $param_Month, $param_Year, $param_PayeeName, $param_Amount);
      ...
      if (mysqli_stmt_execute($stmt))
      {
        ...
        echo 'window.location.href = "sk_elecBillStat.php";';
        echo '</script>';
      }
      // Otherwise, show alert
      else
      {
        echo '<script>alert("You have already paid electricity bill for this month!")</script>';
      }
    }
    //mysqli_stmt_close($stmt);
  }
...
<form action="" method="post">
  ...
  <div class="form-row">
    ...
    <div class="form-group col-md-6">
      ...
      <input type="text" class="form-control" name="PayeeName" id="inputPayeeName" placeholder="Please enter Payee Name">
    </div>
    <!-- email -->
    <div class="form-group col-md-6">
