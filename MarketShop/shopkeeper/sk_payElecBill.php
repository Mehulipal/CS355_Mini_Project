...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
  ...
  if($stmt)
  {
    ...
    if(mysqli_stmt_execute($stmt))
    {
      ...
      // Otherwise, set variable $SID, $Month, $Year
      else
      {
        $SID = trim($_POST['SID']);
  ...
  // insert electricity bill payment details into the database
  ...
  if(empty($SID_err) && empty($Month_err) && empty($Year_err))
  {
    ...
    $sql = "INSERT INTO request_elec_bill (SID, PayForWhichMonth, PayForWhichYear, PayeeName, Amount, SkID_requester) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $sql);
    if ($stmt)
    {
