...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
  ...
  // The request to add the shopkeeper is now sent to ADMIN
  if(empty($SID_err))
  {
    // Prepare mysqli query
    $sql = "INSERT INTO request_add_sk (_SkName, _AadharID, _Address, _Gender, _Contact, _SID, SkID_requester) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $sql);
    if ($stmt)
    {
      mysqli_stmt_bind_param($stmt, "sssssii", $param_SkName, $param_AadharID, $param_Address, $param_Gender, $param_Contact, $param_SID, $param_SkID);
  ...
  // Incase of errors, adding shop fails and alerts are shown 
  else
  {
    if(!empty($SID_err))
    {
...
<form action="" method="post">
  <div class="form-group">
    ...
    <input type="text" class="form-control" name="SID" id="inputSID" value="<?php echo $_SESSION['SID']?>" readonly>
  </div>
  <div class="form-group">
  ...
  <div class="form-group">
    ...
    <input type="text" class="form-control" name="Contact" id="inputContact" placeholder="Contact (10 digits)"  title="Mobile no should be of 10 digits" pattern="[1-9]{1}[0-9]{9}">
  </div>
