...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
  ...
  if(empty($err))
  {
    // Prepare mysqli query to select the shopkeeper details
    ...
    if(mysqli_stmt_execute($stmt))
    {
      ...
      // If SkID exists in database, continue to login
      if(mysqli_stmt_num_rows($stmt) == 1)
      {
        ...
        if(mysqli_stmt_fetch($stmt))
        {
          ...
          {
            // Prepare mysqli query to select the shop details where the shopkeeper works
            ...
            // This means the password is correct. Allow employee to login...
            session_start();
      ...
      // If SkID doesn't exist in database, login fails
      else
      {
